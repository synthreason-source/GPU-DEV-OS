...
/* ?? ADD THESE NEW EDITOR COLORS HERE ?????????????????????????????????? */
/* ?? Theme and Color Definitions ?????????????????????????????????? */
#define COLOR_EDITOR_BG     COLOR_BGRA(22,14,12)   /* Custom editor background */
#define COLOR_EDITOR_LINE   COLOR_BGRA(32,20,18)   /* Alternating line background */
#define COLOR_CURSOR        COLOR_BGRA(0,255,136)  /* Cursor block color */
