...
static const UINT8 Font8x16[128][16] = {
 ...
 {0x00,0x00,0x00,0x00,0x00,0x18,0x3C,0x18,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},
 {0xFF,0xFF,0xFF,0xFF,0xFF,0xE7,0xC3,0xE7,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF},
...
VOID FontDrawString(GPU_FB *Fb, UINT32 X, UINT32 Y,
    ...
    while (*Str) {
        if (*Str=='\n') { CurX=X; CurY+=FONT_H; }
        else { FontDrawChar(Fb,CurX,CurY,*Str,FgColor,BgColor,FALSE); CurX+=FONT_W; }
        Str++;
    }
}
        ...
        if ((UINT8)*Str < 128) {
            for (UINT32 Row=0; Row<FONT_H; Row++) {
                ...
                for (UINT32 Col=0; Col<FONT_W; Col++) {
                    ...
                    GfxFillRect(Fb, CurX+Col*Scale, Y+Row*Scale, Scale, Scale, C);
                }
            }
        }
        CurX+=FONT_W*Scale; Str++;
    }
}
