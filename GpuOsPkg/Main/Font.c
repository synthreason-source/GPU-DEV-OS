/**
 * Font.c — 8x16 PC BIOS-style bitmap font, public domain glyphs
 * Each entry: 16 bytes, one per row, bit7=leftmost pixel.
 */
#include "Font.h"

/* ------------------------------------------------------------------ */
/* 8x16 Bitmap font table — 128 glyphs (ASCII 0x00–0x7F)             */
/* Derived from the classic CP437/VGA 8x16 BIOS font (public domain)  */
/* ------------------------------------------------------------------ */
static const UINT8 Font8x16[128][16] = {
 ...
 /* 0x07 */ {0x00,0x00,0x00,0x00,0x00,0x18,0x3C,0x18,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},
 ...
 /* 0x1E */ {0x00,0x00,0x00,0x10,0x38,0x38,0x7C,0x7C,0xFE,0xFE,0x00,0x00,0x00,0x00,0x00,0x00},
 /* 0x1F */ {0x00,0x00,0x00,0xFE,0xFE,0x7C,0x7C,0x38,0x38,0x10,0x00,0x00,0x00,0x00,0x00,0x00},
 ...
 /* 0x30 */ {0x00,0x00,0x38,0x6C,0xC6,0xC6,0xD6,0xD6,0xC6,0xC6,0x6C,0x38,0x00,0x00,0x00,0x00}, /* 0 */
 ...
 /* 0x3B */ {0x00,0x00,0x00,0x00,0x18,0x18,0x00,0x00,0x00,0x18,0x18,0x18,0x30,0x00,0x00,0x00}, /* ; */
 ...
 /* 0x4B */ {0x00,0x00,0xE6,0x66,0x6C,0x6C,0x78,0x78,0x6C,0x66,0x66,0xE6,0x00,0x00,0x00,0x00}, /* K */
 ...
 /* 0x66 */ {0x00,0x00,0x1C,0x36,0x32,0x30,0x78,0x30,0x30,0x30,0x30,0x78,0x00,0x00,0x00,0x00}, /* f */
 ...
 /* 0x76 */ {0x00,0x00,0x00,0x00,0x00,0x66,0x66,0x66,0x66,0x66,0x3C,0x18,0x00,0x00,0x00,0x00}, /* v */
 ...
 /* 0x7F */ {0x00,0x00,0x00,0x00,0x10,0x38,0x6C,0xC6,0xC6,0xFE,0x00,0x00,0x00,0x00,0x00,0x00}, /* DEL */
};

VOID FontDrawChar(GPU_FB *Fb, UINT32 X, UINT32 Y,
                  CHAR8 Ch, UINT32 FgColor, UINT32 BgColor,
                  BOOLEAN Transparent) {
    if ((UINT8)Ch >= 128) return;
    for (UINT32 Row = 0; Row < FONT_H; Row++) {
        UINT8 Bits = Font8x16[(UINT8)Ch][Row];
        for (UINT32 Col = 0; Col < FONT_W; Col++) {
            if (Bits & (0x80 >> Col))
                GfxPutPixel(Fb, X + Col, Y + Row, FgColor);
            else if (!Transparent)
                GfxPutPixel(Fb, X + Col, Y + Row, BgColor);
        }
    }
}
...
VOID FontDrawString(GPU_FB *Fb, UINT32 X, UINT32 Y,
    ...
    UINT32 CurX = X, CurY = Y;
    while (*Str) {
        if (*Str == '\n') {
            CurX = X;
            CurY += FONT_H;
        } else {
            FontDrawChar(Fb, CurX, CurY, *Str, FgColor, BgColor, FALSE);
            CurX += FONT_W;
        }
        Str++;
    }
...
VOID FontDrawStringScaled(GPU_FB *Fb, UINT32 X, UINT32 Y,
    ...
    while (*Str) {
        if ((UINT8)*Str < 128) {
            for (UINT32 Row = 0; Row < FONT_H; Row++) {
                UINT8 Bits = Font8x16[(UINT8)*Str][Row];
                for (UINT32 Col = 0; Col < FONT_W; Col++) {
                    UINT32 C = (Bits & (0x80 >> Col)) ? FgColor : BgColor;
                    GfxFillRect(Fb,
                        CurX + Col * Scale,
                        Y    + Row * Scale,
                        Scale, Scale, C);
